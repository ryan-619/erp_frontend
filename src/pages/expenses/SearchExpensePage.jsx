...
//
// Purpose:
// Search and filter expense records with export capabilities.
//
// Data Source:
// expenses.service.js
//
...
export default function SearchExpensePage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <Breadcrumbs items={[{ label: 'Home', to: '/dashboard' }, { label: 'Expenses' }, { label: 'Search Expense' }]} />
      <PageHeader
        title="Search Expense"
        description="Search and filter expense records with export."
        icon={Search}
      />
      ...
      <FilterBar>
        <SearchBar value={search} onChange={setSearch} placeholder="Search by head or note…" className="max-w-sm" autoFocus />
        <div className="flex flex-wrap items-center gap-2">
          <ExportButtons rows={rows} columns={EXPORT_COLS} filename="expense-search" />
      ...
      {isLoading ? (
        ...
        <NoData title="No expense records found" description="Try adjusting your search or filters." />
      ) : (
        <DataTable
          columns={columns}
          data={rows}
