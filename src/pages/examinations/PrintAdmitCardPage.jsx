...
// Print Service: examinationService.printAdmitCard()
// ====================================================================
...
export default function PrintAdmitCardPage() {
  ...
  const generateAdmitCardHTML = (studentData) => {
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .admit-card { border: 2px solid #333; padding: 20px; text-align: center; }
        ...
        <body>
          <div class="admit-card">
            ...
            <div class="footer">
              <p>This is a computer-generated admit card. Please bring this card to the examination center.</p>
  ...
  const handlePDFResponse = (jsonData) => {
  ...
  const handlePrint = async (student) => {
    ...
    try {
      const response = await examinationService.printAdmitCard(studentId)
      ...
      if (blob) {
        ...
        if (jsonData.success && jsonData.data) {
          printAdmitCard(generateAdmitCardHTML(jsonData.data))
          toast({ title: 'Success', description: 'Admit card generated for printing' })
          return
        }

        if (jsonData.url) {
          window.open(jsonData.url, '_blank')
          toast({ title: 'Success', description: 'Admit card opened' })
          return
        }

        if (jsonData.pdf) {
          handlePDFResponse(jsonData)
          toast({ title: 'Success', description: 'Admit card opened' })
          return
        }

        toast({ title: 'Success', description: jsonData.message || 'Admit card generated' })
      } else if (response.data?.url) {
        window.open(response.data.url, '_blank')
        toast({ title: 'Success', description: 'Admit card opened' })
      } else {
        toast({ title: 'Success', description: 'Admit card generated' })
      }
    } catch (err) {
      toast({ title: 'Error', description: err.message || 'Failed to generate admit card', variant: 'destructive' })
    } finally {
      setPrintingId(null)
    }
  }
    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      {isLoading ? (
        ...
        <NoData
          ...
          description="No students are available for admit card printing."
        />
      ) : (
        <DataTable
          columns={columns}
